...
from keras.layers import Input, LocallyConnected1D, ZeroPadding1D, Add, Reshape, Flatten, Lambda
...
outputfilename = log_path + 'tH2LCL' + str(k_multigrid) + 'Nc' + str(N_cnn) + 'Al' + str(alpha)
...
# ------- LCR, LCK, LCI layers, see arXiv:1808.02376 for details ------------
def LCR(X, Nin, al_in, Nout, al_out, act_fun):
    return LocallyConnected1D(al_out, Nin//Nout, strides=Nin//Nout, activation=act_fun)(X)

def LCK(X, Nin, al_in, al_out, w, act_fun):
    tmp = ZeroPadding1D(w//2)(X)
    return LocallyConnected1D(al_out, w, activation=act_fun)(tmp)

def LCI(X, Nin, al_in, al_out, act_fun):
    return LocallyConnected1D(al_out, 1, activation=act_fun)(X)
...
for i in range(0, N_cnn-1):
    uad = LCK(uad, 2**L, m, m, 2*n_b_ad+1, 'relu')

uad = LCK(uad, 2**L, m, m, 2*n_b_ad+1, 'linear')
uad = Flatten()(uad)

# === far field part
Vv_list = []
Vv = LCR(Ipt, n_input, 1, 2**L, alpha, 'linear')
Vv_list.insert(0, Vv)
for ll in range(L-1, 1, -1):
    Vv = LCR(Vv, 2**(ll+1), alpha, 2**ll, alpha, 'linear')
    Vv_list.insert(0, Vv)
...
for ll in range(2, L+1):
    ...
    for k in range(0, N_cnn):
        MVv = LCK(MVv, 2**ll, alpha, alpha, w, 'relu')
...
for ll in range(2, L):
    ...
    chi = LCI(chi, 2**ll, alpha, 2*alpha, 'linear')
    chi = Reshape((2**(ll+1), alpha))(chi)

chi = Add()([chi, MVv_list[L-2]])
chi = LCI(chi, 2**L, alpha, m, 'linear')
chi = Flatten()(chi)
...
tr_os = open('trainresultH2LC.txt', "a")
